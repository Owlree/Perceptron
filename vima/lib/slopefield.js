...
var SlopeField = /** @class */ (function (_super) {
    ...
    function SlopeField(slopeFunctionStr, bounds) {
        ...
        var hstep = Math.abs(bounds.left - bounds.right) / 50;
        var vstep = Math.abs(bounds.bottom - bounds.top) / 30;
        for (var i = bounds.left; i <= bounds.right; i += hstep) {
            for (var j = bounds.bottom; j <= bounds.top; j += vstep) {
                var line_1 = new paper.Path.Line(new paper.Point(i - 0.025, j), new paper.Point(i + 0.025, j));
                line_1.rotate(180 / Math.PI * Math.atan2(_this._slopeFunction.evaluate({ x: i, y: j }), 1));
                _this._group.addChild(line_1);
            }
        }
        var current = new paper.Point(0.5, 0.5);
        var segments = [new paper.Segment(current)];
        var prevAngle = undefined;
        for (var i = 0; i < 500; ++i) {
            segments.push(new paper.Segment(current));
            var line_2 = new paper.Path.Line(new paper.Point(current.x, current.y), new paper.Point(current.x + 0.05, current.y));
            var angle = Math.atan2(_this._slopeFunction.evaluate({ x: current.x, y: current.y }), 1);
            ...
            line_2.rotate(180 / Math.PI * angle);
            var vec = new vector2_1.Vector2(Math.cos(angle) * 0.1, Math.sin(angle) * 0.1);
            prevAngle = angle;
            current = new paper.Point(current.x + vec.x, current.y + vec.y);
            _this._group.addChild(line_2);
        }
        var line = new paper.Path.Line(new paper.Point(bounds.left, 0), new paper.Point(bounds.right, 0));
...
//# sourceMappingURL=slopefield.js.map